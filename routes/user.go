...
func Register(ctx iris.Context) {
	...
	userExists, userExistsErr := getAndHandleUserExist(&newUser, userInput.Email)
	...
	if userExists == true {
		utils.CreateEmailAlreadyRegisted(ctx)
		return
	}
...
func Login(ctx iris.Context) {
	...
	userExists, userExistsErr := getAndHandleUserExist(&existingUser, userInput.Email)
	if userExistsErr != nil {
		utils.CreateInternalServerError(ctx)
		return
	}
...
func FacebookLoginOrSignUp(ctx iris.Context) {
	...
	if facebookBody.Email != "" {
		...
		userExists, userExistsErr := getAndHandleUserExist(&user, facebookBody.Email)
...
func GoogleLoginOrSignUp(ctx iris.Context) {
	...
	if googleBody.Email != "" {
		var user models.User
		userExists, userExistsErr := getAndHandleUserExist(&user, googleBody.Email)
...
func AppleLoginOrSignUp(ctx iris.Context) {
	...
	//the JWK.Keyfunc method will automatically select the key with the matching kid (if present) and return its public key as the correct Go type
	...
	if email != "" {
		var user models.User
		userExists, userExistsErr := getAndHandleUserExist(&user, email)
...
func ForgotPassword(ctx iris.Context) {
	...
	var user models.User
	userExists, userExistsErr := getAndHandleUserExist(&user, emailInput.Email)
	...
	if userExists {
		if user.SocialLogin {
			utils.CreateError(iris.StatusUnauthorized, "Credetnials Error", "Social Login Account", ctx)
			return
		}

		//send email

		link := "exp://192.168.1.2:19000/--/resetpassword/"
...
func GetUserSavedProperties(ctx iris.Context) {
	...
	propertiesExists := storage.DB.Where("id IN ?", savedProperties).Find(&properties)

	if propertiesExists.Error != nil {
		utils.CreateInternalServerError(ctx)
		return
	}
...
func AlterUserSavedProperties(ctx iris.Context) {
	...
	validPropertyID := GetPropertyAndAssosiationsByPropertyID(propertyID, ctx)
...
func getAndHandleUserExist(user *models.User, email string) (exists bool, err error) {

...
func GetPropertiesByUserID(ctx iris.Context) {
	...
	propertiesExists := storage.DB.Preload(clause.Associations).Where("user_id = ?", id).Find(&properties)

	if propertiesExists.Error != nil {
		utils.CreateError(
			iris.StatusInternalServerError,
			"Error", propertiesExists.Error.Error(), ctx)
		return
	}
...
func GetPropertyAndAssosiationsByPropertyID(id string, ctx iris.Context) *models.Property {
...
func GetPropertiesByBoundingBox(ctx iris.Context) {
	//make sure the body of the requiest is valid
	...
	//quering the database to find properties that are within that boundingbox and are on the market
	var properties []models.Property
	storage.DB.Preload(clause.Associations).Where("lat >= ? AND lat <= ? AND lng >= ? AND lng <= ? AND on_market = true",
		boundingBox.LatLow, boundingBox.LatHigh, boundingBox.LngLow, boundingBox.LngHigh).Find(&properties)
	//return those properties to the user
	ctx.JSON(properties)
}
